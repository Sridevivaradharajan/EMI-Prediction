...
def load_models():
    ...
    try:
        ...
        clf_files = [f for f in os.listdir(models_dir) 
                    if 'classification_model' in f and f.endswith('.pkl')]
        clf_model = joblib.load(os.path.join(models_dir, clf_files[0]))
        
        # Load regression model
        reg_files = [f for f in os.listdir(models_dir) 
                    if 'regression_model' in f and f.endswith('.pkl')]
        reg_model = joblib.load(os.path.join(models_dir, reg_files[0]))
        
        # Load label encoder
        label_encoder = joblib.load(os.path.join(models_dir, 'GLOBAL_LABEL_ENCODER.pkl'))
        ...
        # Load feature names
        with open(os.path.join(models_dir, 'classification_features.json'), 'r') as f:
            clf_features = json.load(f)
        
        with open(os.path.join(models_dir, 'regression_features.json'), 'r') as f:
...
def prepare_classification_input(input_data, feature_names, preprocessing_type):
    ...
    CRITICAL: Maintains exact feature order from training
    """
    ...
    # Step 2: Add missing features (for one-hot encoded columns that don't appear)
    for col in feature_names:
        if col not in df_encoded.columns:
            df_encoded[col] = 0
    
    # Step 3: CRITICAL - Select features in EXACT training order
    df_encoded = df_encoded[feature_names]
    
    # Step 4: Add log-transformed features ONLY if preprocessing is 'linear'
...
# ============================================================================
# PREDICTION FUNCTIONS (CORRECTED)
# ============================================================================
...
def predict_eligibility(input_data, models):
    ...
    CRITICAL FIXES:
    1. Tree models DON'T use scaler
    2. Linear models USE scaler
    3. Feature order maintained exactly as in training
    """
    try:
        # Step 1: Engineer features
        input_with_features = engineer_features(input_data)
        
        # Step 2: Get preprocessing type
        preprocessing_type = models['metadata']['classification']['preprocessing_type']
        
        # Step 3: Prepare input (with correct feature order)
        ...
        # Step 4: Apply scaling ONLY for linear models
        if preprocessing_type == 'linear':
            # LINEAR MODELS (Logistic Regression) - USE SCALER
            ...
            # TREE MODELS (XGBoost, Random Forest) - NO SCALER
            X_clf_final = X_clf
        ...
        # Step 7: Get class probabilities
        class_probs = {}
...
def predict_emi_amount(input_data, models):
    ...
    try:
        ...
        # Step 4: Apply scaling ONLY for linear models
        if preprocessing_type == 'linear':
            # LINEAR MODELS (Linear Regression) - USE SCALER
        ...
        return max(0, emi_prediction)  # Ensure non-negative
    except Exception as e:
